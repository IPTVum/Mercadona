...
export default async function StaticPage({ params }: StaticPageProps) {
  ...
  const title = page?.title || defaultPage?.title || ''
  const description = page?.description || defaultPage?.description || ''
              ...
              .map(([slug, page]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-gray-700 text-center">{page.title}</span>
                  </Link>
                )
              })}
          </div>
