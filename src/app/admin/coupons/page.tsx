...
export default function AdminCouponsPage() {
  return (
    <Suspense fallback={<div><h1 className="text-2xl md:text-3xl font-bold mb-8">Coupons</h1><div className="flex items-center gap-2 text-gray-500"><Loader2 className="animate-spin" size={20} /> Loading...</div></div>}>
      <CouponsContent />
    </Suspense>
  )
}
...
function CouponsContent() {
  ...
  if (loading) {
    return (
      <div>
        <h1 className="text-2xl md:text-3xl font-bold mb-8">Coupons</h1>
    ...
    return (
      <div>
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold">{editId ? 'Edit Coupon' : 'New Coupon'}</h1>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
          ...
          <div className="flex gap-3 mt-8">
            <button onClick={handleSave} disabled={saving} className="btn-primary disabled:opacity-50">
              {saving ? <><Loader2 className="animate-spin" size={20} /> Saving...</> : <><Save size={20} /> {editId ? 'Update Coupon' : 'Create Coupon'}</>}
            </button>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <Link href="/admin/coupons?action=new" className="btn-primary"><Plus size={20} /> Create Coupon</Link>
      </div>
      <div className="bg-white rounded-xl shadow-sm">
        <div className="p-4 border-b border-gray-200">
          <div className="relative max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input type="text" placeholder="Search coupons..." className="input pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
                ...
                <tr key={coupon.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyCode(coupon.code)} className="p-1 hover:bg-gray-200 rounded" title="Copy"><Copy size={14} /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-medium">{coupon.discount_type === 'percentage' ? `${coupon.discount_value}%` : formatPrice(coupon.discount_value)}</span>
                  </td>
                  <td className="px-6 py-4 text-sm">{coupon.min_order_amount ? formatPrice(coupon.min_order_amount) : 'None'}</td>
                  <td className="px-6 py-4 text-sm">{coupon.used_count ?? 0} / {coupon.max_uses || '∞'}</td>
                  <td className="px-6 py-4 text-sm">{coupon.expires_at ? formatDate(coupon.expires_at) : 'No expiry'}</td>
                  <td className="px-6 py-4">
                    <span className={`badge ${coupon.is_active ? 'badge-success' : 'badge-danger'}`}>{coupon.is_active ? 'Active' : 'Inactive'}</span>
                  </td>
        ...
        {filtered.length === 0 && (
          <div className="p-12 text-center text-gray-500">
            ...
            <Link href="/admin/coupons?action=new" className="mt-4 inline-block text-primary-600 hover:text-primary-700">Create your first coupon</Link>
          </div>
        )}
      </div>
    </div>
