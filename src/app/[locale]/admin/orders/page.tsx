...
export default function AdminOrdersPage() {
  ...
  const printInvoice = (order: Order) => {
    ...
    w.document.write(`
      <!DOCTYPE html>
      <html><head><title>${escHtml(t('invoice'))} #${escHtml(order.id.slice(0, 8))}</title>
      <style>
      ...
      <div class="header">
        <div><h1>${escHtml(t('invoice').toUpperCase())}</h1><p class="id">${escHtml(t('table.orderId'))} #${escHtml(order.id.slice(0, 8))}</p></div>
        <div style="text-align:right"><strong>${escHtml(siteName)}</strong><br>${new Date(order.created_at).toLocaleDateString()}</div>
      </div>
      <div class="info">
        <div class="info-box"><h3>${escHtml(t('billTo'))}</h3><p>${escHtml(addr.full_name || addr.name || order.email)}<br>${escHtml(addr.address || '')}<br>${escHtml(addr.city || '')}, ${escHtml(addr.state || '')} ${escHtml(addr.zip_code || addr.zip || '')}</p></div>
      ...
      <div class="totals">
        <div class="row"><span>${escHtml(t('subtotal'))}</span><span>${formatPrice(order.subtotal)}</span></div>
        <div class="row"><span>${escHtml(t('shipping'))}</span><span>${order.shipping_cost ? formatPrice(order.shipping_cost) : escHtml(t('free'))}</span></div>
        ${order.tax ? `<div class="row"><span>${escHtml(t('tax'))}</span><span>${formatPrice(order.tax || 0)}</span></div>` : ''}
        ${order.discount ? `<div class="row"><span>${escHtml(t('discount'))}</span><span>-${formatPrice(order.discount)}</span></div>` : ''}
        <div class="total row"><span>${escHtml(t('table.total'))}</span><span>${formatPrice(order.total)}</span></div>
      </div>
      <div class="footer">${escHtml(t('thankYou'))} &mdash; ${escHtml(siteName)}</div>
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-4 bg-gray-50">
                        <div className="grid md:grid-cols-3 gap-4 text-sm">
                          <div>
                            <p className="font-medium text-gray-500 mb-1">{t('orderDetails')}</p>
                            <p>{t('subtotal')}: {formatPrice(order.subtotal)}</p>
                            <p>{t('shipping')}: {order.shipping_cost ? formatPrice(order.shipping_cost) : t('free')}</p>
                            <p>{t('tax')}: {order.tax ? formatPrice(order.tax) : '-'}</p>
                            <p>{t('discount')}: {order.discount ? formatPrice(order.discount) : '-'}</p>
                            <p>{t('payment')}: {order.payment_method || t('na')}</p>
                            {order.tracking_number && <p>{t('tracking')}: {order.tracking_number}</p>}
                          </div>
                          <div>
                            <p className="font-medium text-gray-500 mb-1">{t('shippingAddress')}</p>
                          ...
                          <div>
                            <p className="font-medium text-gray-500 mb-1">{t('notes')}</p>
                            <p>{order.notes || t('noNotes')}</p>
                            {order.estimated_delivery && <p className="mt-2">{t('estimatedDelivery')}: {formatDate(order.estimated_delivery)}</p>}
                          </div>
                        </div>
                      </td>
                    </tr>
